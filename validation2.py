...
def validate_structure(cte_file_data):

	
	if re.findall(r'[<>]', cte_file_data) != [] : print "angle-bracket problem"
	if re.findall(r'\^', cte_file_data) != [] : print "caret problem"
	
	problem_brackets = find_problem_brackets(cte_file_data)
	if problem_brackets != '':
		choice = raw_input("see all? (Y)")
		if choice == "Y": 
			f2 = open(fn[:-4] + '_bracketless.txt', 'w')
			f2.write(cte_file_data)
			f2.close()
		view_problems(problem_brackets, cte_file_data, fn)
	final_check_clean_surroundings(cte_file_data, fn)
...
def find_problem_brackets(content, verbose):
	...
	just_brackets = re.sub('[^\[\]{}\(\)<>]','',content)
	minus_valid_chapter_identifiers = re.sub('{[^{}]*?}','',just_brackets)
	minus_valid_folio_breaks = re.sub(folio_break_regex,'',minus_valid_chapter_identifiers)

	removing_remaining_valid_pairs = minus_valid_folio_breaks
	# this is now () [] <>, which can be nested in any way besides overlapping
	# approach by iteratively removing any valid pair found anywhere in the string
	while re.findall(u'\(\)|\[\]|<>',removing_remaining_valid_pairs) != []:
		removing_remaining_valid_pairs = re.sub(u'\(\)|\[\]|<>','',removing_remaining_valid_pairs)

	remaining_problem_brackets = removing_remaining_valid_pairs

	if remaining_problem_brackets != '':
		print "set of problem brackets:", remaining_problem_brackets
	return remaining_problem_brackets
...
def validate_structure(content, verbose=False):
...
# 	f = open(fn,'r'); content = f.read(); f.close()
def validate_content(content, verbose=False):
	content = content.decode('utf-8')
